...
const AGENT_REGISTRY_ADDRESS = (process.env.AGENT_REGISTRY_ADDRESS || '0x643194B69E66F4c85A5994f71a7dE7374092E146') as `0x${string}`;
...
async function main() {
    const privateKey = process.env.ARC_AGENT_PRIVATE_KEY || process.env.PRIVATE_KEY;
    const manifestUrl = process.env.AGENT_MANIFEST_URL || 'https://diversifiapp.vercel.app/.well-known/agent.json';
    const isMainnet = process.env.NETWORK === 'mainnet';

    if (!privateKey) {
        console.error('❌ Error: ARC_AGENT_PRIVATE_KEY or PRIVATE_KEY not found in .env');
        process.exit(1);
    }
    ...
    const manifestPath = path.join(process.cwd(), 'public', '.well-known', 'agent.json');
    if (!fs.existsSync(manifestPath)) {
        console.error('❌ Error: agent.json manifest not found at public/.well-known/agent.json');
        process.exit(1);
    }

    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    const capabilities = manifest.capabilities.map((c: any) => c.id);
