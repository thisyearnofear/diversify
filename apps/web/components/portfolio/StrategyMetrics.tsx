...
// FinancialStrategy is defined in the shared package's strategy types and
// re-exported via `@/context/app/types`. The deep import avoids pulling the
// shared package barrel (and its AI SDK chain) into the overview bundle —
// matches the pattern in `components/swap/TokenSelector.tsx`.
...
export default function StrategyMetrics({ portfolioData }: StrategyMetricsProps) {
    // Wave 5 refactor: `useStrategy()` / `StrategyContext` is superseded by
    // `useProtectionProfile()` (the protection profile is the single source of
    // truth for `philosophy` — the old `financialStrategy` localStorage key has
    // been migrated to `config.philosophy` on the profile).
    const { config } = useProtectionProfile();
    const financialStrategy = config.philosophy;
