...
const USER_GOALS: Array<{ value: UserGoal; label: string; icon: string; description: string }> = [
  { value: 'inflation_protection', label: 'Hedge Inflation', icon: '🛡️', description: 'Protect against currency devaluation' },
  { value: 'geographic_diversification', label: 'Diversify Regions', icon: '🌍', description: 'Spread risk across economies' },
  { value: 'rwa_access', label: 'Access Gold/RWA', icon: '🥇', description: 'Hold real-world assets' },
  { value: 'exploring', label: 'Just Exploring', icon: '🔍', description: 'Learn about protection' },
];
...
const RWA_ASSETS = [
  {
    ...
    description: "Tokenized physical gold backed 1:1 by London Good Delivery gold bars held in Brink's vaults.",
...
export default function ProtectionTab({
  ...
  const { config, updateConfig, advice, portfolioAnalysis, isAnalyzing } = useWealthProtectionAgent();
  ...
  const hasProfile = profileStep >= 3 || (config.userGoal && config.riskTolerance && config.timeHorizon);
  ...
  const handleExecuteSwap = (targetToken: string, fromToken?: string, amount?: string) => {
    ...
    navigateToSwap({
      ...
      toToken: targetToken,
      amount: swapAmount,
      reason: advice?.reasoning || `Swap to ${targetToken} for ${USER_GOALS.find(g => g.value === config.userGoal)?.label || 'wealth protection'}`
    });
  };
      ...
      <div className="space-y-4">
        <Card className="bg-gradient-to-br from-indigo-600 to-purple-700 text-white p-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h3 className="text-xl font-black uppercase tracking-tight">Protection Engine</h3>
        ...
        <Card className="bg-gray-50 border-dashed border-2 p-4">
          ...
          <div className="grid grid-cols-2 gap-2 opacity-50">
            {USER_GOALS.map((goal) => (
              <div key={goal.value} className="p-3 bg-white border border-gray-200 rounded-xl text-center">
                <div className="text-xl mb-1">{goal.icon}</div>
                <div className="text-[10px] font-black uppercase text-gray-900">{goal.label}</div>
              </div>
            ))}
          </div>
        </Card>
    ...
    <div className="space-y-4">
      ...
      <Card padding="p-0" className="overflow-hidden shadow-xl">
        <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-6 text-white">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h3 className="text-xl font-black uppercase tracking-tight">Protection Engine</h3>
              <p className="text-indigo-100 text-xs font-bold opacity-80 mt-1">
                {hasProfile ? 'Personalized protection active' : 'Set your protection profile'}
              </p>
            </div>
        ...
        <div className="p-4 space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-gray-500">
                    {config.riskTolerance} • {TIME_HORIZONS.find(t => t.value === config.timeHorizon)?.label}
                  </div>
                </div>
              </div>
              <button
          ...
          {analysis && (
            <ProtectionScore
              score={Math.round((analysis.diversificationScore + (100 - analysis.weightedInflationRisk * 5)) / 2)}
              factors={[
                { 
                ...
                { 
                  ...
                  status: analysis.tokenCount > 0 ? `${analysis.tokenCount} tokens` : 'No data', 
                  icon: '💰' 
                },
                { 
      ...
      {totalValue > 0 && (
        ...
        >
          <MultichainPortfolioBreakdown regionData={regionData} totalValue={totalValue} />
        </CollapsibleSection>
      )}
            ...
            {RWA_ASSETS.filter(a => a.symbol === showAssetModal).map(asset => (
              <div key={asset.symbol}>
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-16 h-16 bg-amber-100 dark:bg-amber-900/30 rounded-2xl flex items-center justify-center text-4xl shadow-inner">
                    🏆
                  </div>
                  <div>
                    <h3 className="font-black text-xl text-gray-900 dark:text-gray-100">{asset.symbol}</h3>
                    <span className="text-[10px] font-black uppercase bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-300 px-2 py-1 rounded-md">
                      {asset.type}
                    </span>
                  </div>
                </div>
