        ...
        <div className="flex-1 flex flex-col">
            <div className="p-8 border-b border-gray-100 dark:border-gray-800/50 bg-white/50 dark:bg-gray-900/50">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xl font-[900] tracking-tight text-gray-900 dark:text-white">
            ...
            <div className="flex-1 flex items-center justify-center p-6 relative overflow-hidden">
                ...
                >
                    <motion.div
                        ...
                        className={`p-8 rounded-[2rem] border-2 transition-all duration-500 relative overflow-hidden ${selected === strategy.id
                            ? 'border-blue-500 bg-white dark:bg-gray-900 shadow-[0_24px_48px_-12px_rgba(37,99,235,0.2)] scale-105'
                            : 'border-gray-100 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 shadow-xl'
                            }`}
                    >
                        {selected === strategy.id && (
                            <div className="absolute top-4 right-4 bg-blue-500 text-white text-[8px] font-black uppercase tracking-tighter px-2 py-0.5 rounded-full shadow-lg shadow-blue-500/50">
                                Selected
                            </div>
                        )}
                        <div className="relative mb-8 flex justify-center text-8xl">
                            {strategy.icon}
                        </div>
                        <h4 className="text-3xl font-[900] text-gray-900 dark:text-white text-center mb-1 tracking-tight">
                            {strategy.name}
                        </h4>
                        {strategy.nativeName && (
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 dark:text-blue-400 text-center mb-6 opacity-80">
                                {strategy.nativeName}
                            </p>
                        )}
                        <p className="text-sm font-bold text-center mb-4 text-gray-600 dark:text-gray-300 leading-relaxed max-w-[240px] mx-auto">
                            {strategy.tagline}
                        </p>
                        <p className="text-xs text-gray-400 dark:text-gray-500 text-center leading-relaxed mb-8 italic">
                        ...
                        <div className="flex flex-wrap gap-1.5 justify-center">
                            {strategy.values.map((value, i) => (
                                <span
                                    key={i}
                                    className={`text-[9px] px-2.5 py-1 rounded-lg font-black uppercase tracking-wider transition-colors duration-500 ${selected === strategy.id
