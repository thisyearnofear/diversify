...
- **API routes**: `POST /api/fx-netting/match` (pool-backed matching + settlement + ledger anchor, returns `poolSize`), `POST /api/fx-netting/intent` (wallet-authenticated intent creation **+ pool persistence**), `GET /api/fx-netting/intent` (list the caller's pool intents)
...
- **Tests pass** (933 at the parity commit; pool/card suites green since). TypeScript + ESLint clean
...
- **Remaining**: settlement execution from net obligations (real cUSD transfers) + Caribbean user/partner evidence (LOI)
