...
- **Settlement execution (2026-08-31, done)**: `models/FxSettlementRecord.ts` + `pages/api/fx-netting/settle.ts` + `packages/shared/src/services/fx-netting/settlement-execution.ts`. Zero-custody (same trust model as HSP): the net DEBTOR sends the cUSD transfer from their own wallet browser-side and submits the txHash; the server verifies the ERC-20 Transfer log ON-CHAIN (right token/debtor/creditor, `amount >= netAmount − ε`) before advancing the settlement + both intents to `settled`, then anchors `FX_SETTLE` to the ledger on the region-canonical chain (fire-and-forget). Idempotent (same txHash re-returns the settled record; different txHash rejected). Region-agnostic: `chainId`/`settlementCurrency` are inherited from the matching engine's region-canonical routing, never hardcoded — the settle RPC resolver fails loudly on a rail without a configured env key rather than verifying against the wrong chain. Debtor can settle later (next session, next day) without a re-match.
