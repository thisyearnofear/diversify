...
- **2026-09-05** — **The settlement-native credit layer ships in embryo — the track's CORE component.** The insight the whole brief points at: the netting pool's own verified settlement history IS the alternative data that cash-flow underwriting needs (IFC/CGAP's "cracking the credit code" thesis), generated natively instead of imported from bank statements. Every settled obligation was already verified on-chain between wallet-signed counterparties — so **the first settled trade builds your credit file**. `credit-profile.ts` (pure scorer): behavioural factors only from verified settlements (debtor completion, settlement speed, counterparty-diverse volume, recency), 300–850 band, provenance on every factor, and a hard thin-file gate — fewer than 3 verified settlements means `score: null`, reported honestly, never invented. `GET /api/fx-netting/credit-profile`: self view (full factors) + privacy-preserving counterparty view (band only); demo/observer/Guardian-liquidity participants are excluded from scoring by construction. **Liquidity bootstrap**: the Guardian seeds the deepest Caribbean corridors (BBD↔JMD, TTD↔JMD) with standing mid-market intents so a real SME's first intent finds a counterparty instead of a dead pool — `guardian-liquidity-` prefixed, credit-ineligible, disclosed in every match response, corridors skipped when no live rate exists rather than seeded fabricated. The cold-start answer is now structural, not aspirational: coordination today underwrites working capital tomorrow, and the first real settlements matter disproportionately — which is exactly why none are simulated. **Verified live on production:** observer probe (walletless) correctly returned `poolSize 0` with no seeding — previews don't mutate the pool; a persisted probe returned *"Guardian standing liquidity seeded 4 corridor(s) at mid-market"*, the pool grew to 5, and the engine matched 3 legs against the seeded book with the bootstrap disclosed in the response. The credit-profile route serves on production behind wallet auth; the netting review card now shows the caller's live credit file (thin-file honesty rendered, never hidden).
- **2026-09-06** — **90-second submission video shipped (`videos/caribbean-submission/`).** A 75.4s narrated cut (River VO + instrumental bed, ElevenLabs; HyperFrames render) covering the full arc the track asks for: personal currency leak (live JMD −10%/5Y read) → values-based protection picker → the savings-to-liquidity turn → a **real posted FX intent matched on camera** (BBD→JMD vs a wallet-signed counterparty intent in the live pool — MATCHED $25,000 at mid-market 0.0127, SAVED ~$1,750 vs the 700bps corridor default) → net-settlement + zero-custody on-chain verification → the settlement-native credit file quoting the live `credit-profile` API verbatim (thin file reported as `score: null`, never invented) → the public proof surface (one ledger `0x3BCf…369C` on 5 mainnets, Agentic ID `0x6815…33D60` on 0G, 380+ RPC-verifiable receipts). Demo-intent cleanup after filming via `scripts/fx-pool-cleanup.ts --pattern dbd5ea7c --apply` (row marked cancelled, audit trail intact — the ephemeral signer key was discarded so it could never settle). Render: `videos/caribbean-submission/renders/caribbean-submission_2026-09-06_14-51-27.mp4` (1m15s, 8.1MB). Suite: **1,261 tests / 155 files, all green**.
