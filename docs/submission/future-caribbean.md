...
- **Liquidity bootstrap (honest cold start)**: the Guardian seeds the deepest Caribbean corridors (BBD↔JMD, TTD↔JMD) with standing mid-market intents so a real SME's first intent finds a counterparty instead of a dead pool — every bootstrap action is disclosed in the match response, seeded corridors without a live rate are skipped rather than fabricated, Guardian liquidity is excluded from credit scoring, and walletless previews never mutate the pool. Guardian liquidity is a standing *quote*, not settlement capital — funded float is the declared next workstream.
