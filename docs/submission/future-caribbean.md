...
- **Settlement-native credit layer (the MSME credit component, in embryo)**: the pool's verified settlement history **is** the alternative data that cash-flow underwriting needs — generated natively by coordination itself rather than imported from fragmented bank statements. This is the underwriting logic the Caribbean already trusts, formalized: the rotating savings club — **partner** in Jamaica, **sou-sou** in Trinidad, **san** in the Dominican Republic — has always lent on the strength of your history in the circle, not on bank statements. The credit file makes that history verifiable and portable. Every settled obligation was verified on-chain between wallet-signed counterparties, so behavioural factors (debtor completion, settlement speed, counterparty-diverse volume, recency) accumulate into a 300–850 credit profile with per-factor provenance. A thin file is reported as a thin file (`score: null` below 3 verified settlements — never invented); a counterparty can see a business's band without its private factors. **The first settled trade builds your credit file.**
- **Liquidity bootstrap (honest cold start)**: the Guardian maintains standing mid-market quotes on the deepest Caribbean corridors (BBD↔JMD, TTD↔JMD) — book depth a participant can price against, disclosed on every match run. Guardian quotes are book depth, never a matching party on live runs: settlement is fully human-to-human (an unfunded agent leg would create an on-chain obligation nobody can settle), and walletless previews never mutate the pool. The Guardian's liquidity *role* becomes funded — and user-owned — in Phase 2.
- **User-funded liquidity provision (Phase 2 design, shipped as design)**: the Guardian never needs its own capital — the network's liquidity comes from its own users. Opt-in savers and SMEs sign a Guardian permission dedicating a slice of their stablecoin balance as netting float; the Guardian fills matched legs **within those signed bounds** (the same permission machinery that already executes rebalances: EIP-712 consent, caps keyed to actual debit, every leg anchored on-chain), and float providers earn a share of the ~700 bps corridor savings their liquidity unlocks. Liquidity, income, and control all stay with participants; the agent coordinates and executes only inside human-signed bounds.
...
**Limitations & risks, managed.** Thin two-sided liquidity is the honest risk for any netting market: the hosted intent pool mitigates it across time, Guardian standing liquidity gives early participants an immediate counterparty, and the residual plan routes unmatched needs to conventional rails rather than pretending. The settlement-native credit layer is an embryo: behavioural files are young by definition, so scores are decision-*support* for lenders, never a standalone underwriting basis, and synthetic participants (demo/observer/Guardian liquidity) are excluded from scoring so preview traffic can never build a credit history. Smart-contract risk is contained because settlement is plain ERC-20 transfers verified post-hoc — there is no vault holding funds. Oracle risk: mid-market rates come from multiple named sources with staleness labels. These limitations are stated in the product itself, not just this document.
