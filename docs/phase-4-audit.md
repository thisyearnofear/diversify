...
**Mitigation in place:** the 2026-06 hardening pass made the app-layer enforcement robust (atomic state, double-execute guard, wei math, dequeue-before-execute, first-execution consent gate — `firstAutoExecutionConfirmed` is now set at GUARDIAN-tier permission grant time and after the first manual rebalance, validator bypass closure, daily-limit clamp). These remain valuable as defense-in-depth even after on-chain enforcement lands; they do not, by themselves, remove server trust.
