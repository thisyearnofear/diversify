...
| **0G DA integration (real)** | Reviewer feedback (Wave 2–3) flagged that docs described a "DA layer" when the implementation is 0G Storage doing double duty for evidence anchoring and agent-state persistence — there is no 0G Data Availability SDK integration. Docs now say this plainly (`architecture.md`, `submission/agentic-workflow.md`). Scoped next-wave work: integrate the actual 0G DA product for evidence submission/retrieval, distinct from the Storage-based evidence anchor. | 0G Bridge, next wave |
