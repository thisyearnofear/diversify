...
### Verified wallets claim G$; daily health alarm; outcome funnel; FX market-closed state (2026-09-25)

Daily G$ claiming now needs only GoodDollar verification — the $1+ swap gate is gone (swaps still build the streak). A daily GitHub Action (`.github/workflows/health-check.yml`, 07:00 UTC + manual) runs `pnpm check-swap-routes` and reads `/api/status` + `/api/agent/status`; low/blocked ledger gas, degraded/stale Guardian runs or a dead route open one `health-alert` issue, which it closes on recovery. Its first run (a Friday night) exposed a real product state: Mento v3 stops quoting when FX markets close (FPMM "FX market is currently closed"; oracle pools "no valid median"). That is now its own `market_closed` class — the ticket says "FX market closed — try again when it reopens" instead of blaming a missing pool, LiFi still fills most stable pairs meanwhile, and the route check reports MARKET-CLOSED rather than failing. `swap_outcome` / `claim_outcome` funnel events (coarse, demo-gated) plus `pnpm funnel-outcomes` make Exchange completion measurable. Also: Signal Lens takes an injected Gateway evaluator (literal `import('ai')` in apps/web; the `new Function` loader is gone), the Hetzner overlay ships only packages the trace left incomplete, the 0G SDK include is scoped to the 30 routes that trace it (standalone 261M → 235M), Arbitrum's picker drops USDY (`UNROUTABLE_SWAP_TOKENS`, re-checked by the route check), and Ask Guardian loses the READY label and crossfades the FAB → panel morph.

### Ledger spend cut to ≈$0.3/month (2026-09-25)

The Guardian heartbeat dropped from every-30-minutes to a deliberate rail cadence owned by the Hetzner crontab: `POST /api/agent/guardian-heartbeat` now takes an optional `rails` list (`primary` | `caribbean` | `apac` | `mirror`; absent = all four; unknown → 400), and the cron sends `{"rails":["primary","caribbean","mirror"]}` every 2 days plus `{"rails":["apac"]}` weekly. Measured per-write costs (Celo ≈$0.004, Arbitrum ≈$0.012, HashKey ≈$0.011, 0G ≈$0.0001) put the old 48-writes/day schedule at ≈$7–10/month and the new one at ≈$0.2–0.35/month — every advisory is still individually anchored, no digest. Same session fixed the write economics around it: per-write gas is now `estimateGas × 1.25` (was a hardcoded 500k limit that priced Celo writes at 0.201 CELO against a real 0.046), underfunded signers skip with a stable `underfunded:` error instead of broadcasting a doomed tx, `/api/status` reports per-chain `ledgerGas` runway, the Hetzner deploy rsyncs the full 104-package runtime closure and smoke-requires every runtime root post-restart, and `check-env-parity.sh` reconciles Vercel/server/required env names. The heartbeat run-freshness window moved to 6 days to match the 2-day cadence.

### Daily G$ claim restored; Ask Guardian docks beside the page on desktop (2026-09-25)

Every G$ claim and face-verification attempt had been failing silently: `GoodDollarService.fromWeb3Provider` built the viem wallet client without an `account`, and `@goodsdks/citizen-sdk`'s `IdentitySDK`/`ClaimSDK` constructors throw on that before any wallet prompt (reproduced against the installed SDK). The failure was then hidden — the fallback `window.open` ran after awaits and was popup-blocked, errors rendered only in Home's claim rail, verify moved to "awaiting" on failure, and the no-wallet path reported success. Now: SDK 1.2.7, account attached, reads over the app's Celo RPC, the wallet is switched (or Celo added) before claiming, `getWalletClaimStatus()` gates `claim()` (unverified wallets get a verify prompt instead of the SDK's hard redirect; already-claimed shows the real next reset from `periodStart`/`currentDay`), linked wallets count as verified (`root ≠ 0`, entitlement on the root), failures toast where the user tapped, and ClaimRail's error line carries a real "Claim on GoodDollar →" link. Face verification opens a named popup synchronously on desktop (re-checks when it closes) and redirects on mobile/MiniPay/Farcaster. A regression test constructs the real SDK from the service's clients. Live read-only check: 4 recent mainnet claimers read as whitelisted + already claimed with the real 12:00 UTC reset. The $1+ swap gate on claiming is unchanged (product decision pending). Separately, Ask Guardian on desktop (≥lg) is a right-docked 420px panel with no scrim: the FAB morphs into it (shared `layoutId`), ⌘K / "/" open and Esc closes, the header carries "Looking at: {tab} · {pair}", Settings/History/New sit behind one ⋯ menu, chips are per-tab questions (tiers on hover), the balance is one quiet footer line, the mascot thinks/nods instead of looping, and at ≥1180px the app column reflows so the panel covers nothing. Mobile keeps the bottom sheet. Files: `gooddollar-service.ts`, `use-streak-rewards.tsx`, `use-claim-flow.tsx`, `ClaimRail.tsx`, `AIChat.tsx`, `FloatingControls.tsx`, `FreemiumPanel.tsx`, `AppShell.tsx`, `VoiceButton.tsx`, `ResearchCheck.tsx`, `constants/tabs.ts`, `design-language.md` §5 (+tests).

### Swap routing repair — Uniswap V3 fixed, Mento SDK v3 adopted, honest Celo quotes (2026-09-25)

CELO→KESm/USDm swaps failed because LiFi has no Celo DEX that routes CELO and `UniswapV3Strategy` had never worked: the Celo router address was Ethereum's (no code on Celo), quotes were sent to the router instead of QuoterV2, fee tier 100 (where CELO liquidity sits) was never tried, and Celo's SwapRouter02 takes a 7-field struct with no deadline. All four are fixed (Arbitrum keeps SwapRouter v1 + its own QuoterV2), with a 3% price-impact guard (1000 CELO→KESm direct would lose ~24%; it now surfaces as `no-route` and the ticket offers "Route via USDm instead →" proactively). Mento now runs through `@mento-protocol/mento-sdk@3.4.0` via one service (`mento-sdk.service.ts`), shared by `MentoSwapStrategy`, the Guardian executor (approve + swap to the v3 Router in one UserOp), `/api/celo/mento-{quote,swap}` and the tradeable-token list — adding FPMM pools (CHFm, EURm, GBPm, JPYm, USDC, USDT ↔ USDm) and single-tx multi-hop (KESm→BRLm, EURm→NGNm). The hand-rolled broker code (`exchange-discovery.ts`, `execution.ts`, `utils/mento-utils.ts`) is gone. Celo quotes come from `SwapOrchestratorService.getEstimate` — static-rate fallbacks removed; `getEstimate` keeps the most specific no-route reason plus `errorClass`. New `pnpm check-swap-routes` (live, read-only) passes 12/12 expectations. Verified: 678 swap/hook/executor/API tests green, tsc + lint clean, `pnpm build` green with the SDK only in lazy chunks; anvil fork executed a Mento KESm→BRLm 2-hop swap (−100 KESm, +3.96 BRLm); `eth_simulateV1` on Celo mainnet state executed approve + Uniswap CELO→USDm (10 CELO → 0.939 USDm). Arbitrum follow-up: syrupUSDC metadata corrected to its on-chain 6 decimals (quotes rendered 1e-12× and sells would have over-sized the transaction); `DirectRWAStrategy` deleted (hardcoded-price estimates, PAXG at $2,650, and an execute that always failed); the Arbitrum quote branch now uses `getEstimate` like Celo. The route check covers 7 Arbitrum pairs (all expectations met; USDC→USDY and 50k USDC→PAXG are honest no-routes).

### Walletless Shield copy, Home connection continuity, Exchange logo coverage (2026-09-25)

The Shield ghost ring now says what the plan reserves (for example, "70% · dollar reserve · plan target"); the philosophy badge names the plan once, the hole keeps the compare affordance, and the button simply says "Connect wallet". Ring ↔ gallery now crossfades in the same grid cell instead of waiting for the outgoing view to disappear first; browser traces during ring-to-gallery and gallery-to-ring transitions showed no blank-opacity frames. On Home, a pending wallet prompt no longer replaces the unconnected Risk Theater — the object stays explorable and the existing connect CTA shows "Connecting…". Exchange's Celo Mento logo registry now covers GHSm, NGNm, XOFm, ZARm, GBPm, CHFm, JPYm, AUDm, and CADm with verified Trust Wallet assets; GoodDollar (`G$`) remains on the branded Coin fallback because no verified public asset was found. A regression test checks Celo token logo coverage and that Mento logo URLs match the configured contracts. Files: `ProtectionPlanRing.tsx`, `ProtectionNotConnected.tsx` (+tests), `OverviewTab.tsx` (+test; removed unused `ConnectingState.tsx`), `token-logos.ts` (+coverage test), `design-language.md`, `exchange-instrument.md`. Verified: 238 test files / 2,148 tests pass; TypeScript and targeted ESLint clean; mobile browser checked at 420×900.

### Guardian goes wallet-native — the vault was never real; ERC-7710 is the only autonomy (2026-09-24)

First-principles cleanup of Guardian execution. The codebase had accumulated **four** execution paths — Circle developer-controlled wallets, a Privy Safe + delegated embedded signer, a Safe4337 provider whose "smart account" was the operator EOA, and the MetaMask ERC-7710 redeemer — none fully provisioned, and the custodial "vault" never held funds anyway (prod: 5 profile docs, zero deposits, zero vault transactions). The current truth: **one-tap user signing is the default** (Guardian proposes → "Review this move →" → Exchange prefilled, user signs; receipt returns to Guardian via `origin: guardian`), and **ERC-7715/7710 is the only autonomous path** — a scoped session account redeems the user's MetaMask Advanced Permission on *their own* smart account, bounds enforced on-chain by the DelegationManager. Deleted: all three custodial providers + `permissionless`, the Privy delegation-consent rail, `/api/vault/{create,deposit,withdraw,balance,fees}`, the deposit verifier, and the fee engine (fees are "under review"; the 0.10% swap spread is documented). MongoDB data untouched — no migrations, no deletes. Autonomy chains are now derived from the installed `@metamask/smart-accounts-kit` (`getSmartAccountsEnvironment` supports 1, 10, 56, 137, 8453, 42161, 42220, 59144 + testnets; intersect `ChainDetectionService` → **Celo 42220, Celo Sepolia 11142220, Arbitrum 42161** — Celo eligibility was verified, not assumed). Routing: Celo → Mento `approve`+`swapIn`; other eligible chains → LI.FI `GET /v1/quote` → `estimate.approvalAddress` approve + `transactionRequest` swap; both legs ride **one atomic UserOp** (`sendBatch` — the old sequential loop could half-land). Fail-closed: missing `GUARDIAN_SESSION_PRIVATE_KEY`/bundler or an ineligible chain → the loop journals `advisory_pending_user_review` (one-tap fallback), never an error. The client grant resolves token+chain from per-chain config (`GRANT_TOKEN_BY_CHAIN`), the self-address session fallback is gone, and "Stronger protection" hides when `NEXT_PUBLIC_GUARDIAN_SESSION_ADDRESS` is unset. The grant's `delegationContext` now persists on `Permission` (PATCH, shape-checked, chain-matched) and `executor.ts` registers `setDelegationContextResolver` at module load so redemption actually resolves — previously the resolver was never wired and the provider could only throw. `GUARDIAN_SESSION_PRIVATE_KEY` added to the test-env signer-key scrub (`SESSION_PRIVATE_KEY` suffix). `/api/status` reports `checks.autonomy` (ERC-7710 configured/not), not a vault block. 2,145 tests pass.
...
The per-user Circle Developer-Controlled "agent fuel" wallet path is removed — DiversiFi does not hold per-user custodial wallets; Guardian execution runs on the user's wallet under signed permissions. This historical implementation note's description of the Protection Balance as the user's own Circle Gateway balance should not be read as proof of a generally available, deployed cross-chain balance product. `CircleService` keeps only what live code uses (unified USDC balance, EIP-3009 helpers, Arc chain resolution); `WalletService`, `ArcAgent`, `BridgeService`, `agent/status`, and the smoke script drop the wallet wiring. Separately, `gateway_batched` receipts now report Circle's settlement id as `settlementId` with `onChainSettled: false` — the on-chain write lands in Circle's later batch, so calling it a settled tx was a lie. The separate `Vault.circleWallet*` fields stay — they back the distinct Celo vault executor, not agent fuel.
...
Historical implementation record — Arc mainnet became live on 2026-09-16; the integration landed in `9987ff7c` (2026-09-24). Production deployment activation remains unconfirmed. `VAULT_PRIVATE_KEY` is a legacy-named server-side settlement signer, not a user-funds vault.
...
| `packages/shared/src/services/settlement-service.ts` | Promote `ZERO_G` to the default `network` parameter in `settleOnChain` via `DEFAULT_SETTLEMENT_NETWORK` (env-driven via `SETTLEMENT_NETWORK`). This was the interim default while Arc mainnet support was pending; Arc mainnet integration landed later in `9987ff7c` (2026-09-24). The default remains ZERO_G/testnet; Arc production activation is unconfirmed. | +8 | CLEAN, DRY |
...
payments settle on Arbitrum mainnet. **Historical note:** Arc mainnet is live and integrated as of 2026-09-24 (`9987ff7c`); production activation remains unconfirmed. 0G mainnet still lacks a verified USDC contract.
...
| **0G Pay** (agent nanopayments) | `packages/shared/src/services/settlement-service.ts` (`SettlementNetwork` includes `ARC` and `ZERO_G`) | Historical status: ZERO_G was interim default; Arc mainnet integration was pending | Document | Switch default to 0G as interim | 0G Pay mainnet settlement (interim until Arc mainnet integration) | Volume dashboard | Arc mainnet integration landed 2026-09-24 (`9987ff7c`); ZERO_G/testnet remains the code default; production Arc activation unconfirmed |
...
credit the merchant's Gateway balance through Circle's batch and do not
appear as an immediate ERC-20 transfer — noted in the response).

#### Privy vault execution wired to the documented signer+bundler flow (2026-09-24)

Historical implementation note: this Privy smart-account provider and its vault execution path were removed on 2026-09-24; the following describes that intermediate implementation, not current Guardian execution. The `privy` smart-account provider previously called APIs that don't exist
