...
| 0:22–0:26| The mobile Protect-Tab mockup, scroll the carousel | "Cards live where the user actually sees them — inside the live app at diversifiapp.vercel.app." |
| 0:26–0:30| Cut to the BEFORE / AFTER Africapitalism Mode frame, slow pan from left device to right | "Same data. Different worldview. The card family is what we shipped — the Mode runtime is where it's going." |
...
## Three surfaces, one definition (now provable)

The `code-connect.manifest.json` now lists **14 components** — 7 square
cards + 7 vertical stories — each bound to the same archetype data in
`components/protection-cards/`. Open it to see the TS → Figma bindings
across both formats. This is the "one definition, three surfaces" claim
made concrete: app card (1080² in app), share image (1080² PNG),
story share (1080×1920 PNG), all driven by the same archetype tokens.
