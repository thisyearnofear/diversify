 ...
 * callers wire in a rate provider (CLI: scripts/fx-drag/rates.ts; API:
 * ./rates-serverless.ts) and rendering.
 *
 * Canonical home: this module is the single source of truth. The CLI
 * (scripts/fx-drag/calc.ts) re-exports it so `npx tsx scripts/fx-drag-report.ts`
 * keeps working, and the x402 gateway imports it via @diversifi/shared.
 *
...
/**
 * Validate cycle structure and dates WITHOUT fetching rates (throws on invalid).
 * Lets a caller reject malformed input before doing paid/expensive work.
 */
export function validateCycles(input: DragInput): void {
  if (!input.cycles?.length) {
    throw new Error('At least one purchase cycle is required');
  }
  for (const cycle of input.cycles) assertValidCycle(cycle);
}
