...
class AutomationService {
     ...
     */
    async processAnalysis(analysis, userEmail, portfolioData) {
        const payload = {
     ...
     */
    async triggerZapierWebhook(payload) {
        try {
            // Use Zapier MCP service for better integration
            const { zapierMCPService } = await Promise.resolve().then(() => __importStar(require('./zapier-mcp-service')));
            if (zapierMCPService.isConfigured()) {
                const success = await zapierMCPService.triggerAutomation(payload.analysis, payload.user.email, 
                // Use actual wallet if available in user preferences or portfolio context, otherwise placeholder
                payload.user.preferences?.walletAddress || '0x0000000000000000000000000000000000000000', payload.user.portfolio);
                if (success) {
     ...
     */
    async triggerMakeWebhook(payload) {
...
View full analysis: ${process.env.NEXT_PUBLIC_APP_URL || 'https://diversifi.app'}
     ...
     */
    async sendViaSendGrid(to, content) {
     ...
     */
    async sendViaResend(to, content) {
        if (!this.config.email.apiKey) {
     ...
     */
    determineNotificationType(analysis) {
        if (analysis.urgencyLevel === 'CRITICAL')
            return 'urgent_action';
        if (analysis.action !== 'HOLD')
            return 'rebalance_alert';
