...
exports.CircleGatewayService = void 0;
...
class CircleGatewayService {
    provider;
    constructor() {
        this.provider = new ethers_1.providers.JsonRpcProvider(process.env.ARC_RPC_URL || 'https://rpc.testnet.arc.network');
    }
    /**
     ...
     */
    async getUnifiedUSDCBalance(walletAddress) {
        try {
            ...
            const totalUSDC = mockChainBalances.reduce((sum, chain) => sum + parseFloat(chain.usdcBalance), 0).toFixed(2);
            const result = {
                totalUSDC,
     ...
     */
    async getUSDCBalanceOnArc(walletAddress) {
        try {
            const usdcContract = new ethers_1.ethers.Contract(config_1.ARC_DATA_HUB_CONFIG.USDC_TESTNET, ['function balanceOf(address) view returns (uint256)'], this.provider);
            const balance = await usdcContract.balanceOf(walletAddress);
            return ethers_1.ethers.utils.formatUnits(balance, 6); // USDC has 6 decimals
        }
        catch (error) {
     ...
     */
    async transferUSDCViaGateway(fromChainId, toChainId, amount, walletAddress) {
        try {
            // Validate chains
     ...
     */
    async verifyGatewayTransaction(transactionId) {
...
exports.CircleGatewayService = CircleGatewayService;
//# sourceMappingURL=circle-gateway.js.map