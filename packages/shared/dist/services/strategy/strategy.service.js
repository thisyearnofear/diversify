...
class StrategyService {
     ...
     */
    static calculateScore(strategy, currentAllocations) {
...
- Target: Equal distribution across continents`;
     ...
     */
    static filterAssets(strategy, assets) {
        const config = this.getConfig(strategy);
        ...
        return assets.filter(asset => {
            return !config.excludeAssets.some(excluded => asset.toUpperCase().includes(excluded.toUpperCase()));
        });
    }
    /**
     * Get recommended assets based on strategy
     */
    static getRecommendedAssets(strategy) {
        const config = this.getConfig(strategy);
     ...
     */
    static getAssetCompliance(strategy, assetSymbol) {
        if (!strategy) {
