...
exports.rwaService = exports.RWAService = exports.RWA_NETWORKS = exports.RWA_TOKENS = void 0;
exports.formatRWAToken = formatRWAToken;
...
class RWAService {
    ...
    getProvider(network) {
        if (!this.providers[network]) {
            const config = exports.RWA_NETWORKS[network];
            if (!config)
            ...
            try {
                this.providers[network] = new ethers_1.ethers.providers.JsonRpcProvider(rpcUrl);
            }
            catch (error) {
     ...
     */
    getAllRWATokens() {
        return Object.values(exports.RWA_TOKENS).flat();
    }
    /**
     * Get RWA tokens by type
     */
    getRWATokensByType(type) {
        return this.getAllRWATokens().filter(token => token.type === type);
    }
    /**
     * Get RWA tokens by network
     */
    getRWATokensByNetwork(network) {
        return exports.RWA_TOKENS[network] || [];
    }
    /**
    ...
    getRWARecommendations(preferences) {
        ...
        tokens = tokens.filter(token => {
            const networkConfig = exports.RWA_NETWORKS[token.chain];
        ...
        tokens = tokens.filter(t => riskMapping[preferences.riskTolerance].includes(t.riskLevel));
        // Sort by efficiency (APY / gas cost ratio for small amounts)
        tokens = tokens.sort((a, b) => {
            const aNetwork = exports.RWA_NETWORKS[a.chain];
            const bNetwork = exports.RWA_NETWORKS[b.chain];
    ...
    async getTokenBalance(tokenAddress, userAddress, network) {
        try {
            ...
            return parseFloat(ethers_1.ethers.utils.formatUnits(balance, decimals));
        }
        catch (error) {
     ...
     */
    async getCurrentAPY(tokenAddress, network) {
        // This would integrate with specific protocol APIs
        // For now, return cached APY from token registry
        const token = this.getAllRWATokens().find(t => t.address.toLowerCase() === tokenAddress.toLowerCase() && t.chain === network);
        return token?.apy || 0;
    }
    /**
     ...
     */
    async getGasEstimate(network, operation) {
        const provider = this.getProvider(network);
        try {
            const gasPrice = await provider.getGasPrice();
            const gasPriceGwei = parseFloat(ethers_1.ethers.utils.formatUnits(gasPrice, 'gwei'));
            ...
            const estimatedCostETH = parseFloat(ethers_1.ethers.utils.formatEther(gasPrice.mul(gasUnits)));
            // Use network's average cost or calculate from current prices
            const networkConfig = exports.RWA_NETWORKS[network];
     ...
     */
    getOptimalNetwork(investmentAmount) {
        const networks = Object.entries(exports.RWA_NETWORKS).map(([key, config]) => ({
            network: key,
            config,
