...
exports.initializeLiFiConfig = initializeLiFiConfig;
exports.initializeLiFiForQuotes = initializeLiFiForQuotes;
exports.isLiFiConfigured = isLiFiConfigured;
exports.resetLiFiConfig = resetLiFiConfig;
exports.ensureWalletConnection = ensureWalletConnection;
exports.validateWalletProvider = validateWalletProvider;
exports.checkExecutionProviders = checkExecutionProviders;
...
function initializeLiFiConfig() {
    ...
    try {
        ...
        const supportedChains = [chains_1.arbitrum, chains_1.celo, chains_1.celoSepolia];
            ...
            providers: [
                (0, sdk_1.EVM)({
                    getWalletClient: async () => {
                        ...
                        const chain = supportedChains.find(c => c.id === chainId) || chains_1.arbitrum;
                        // Create a wallet client using the browser's ethereum provider
                        const walletClient = (0, viem_1.createWalletClient)({
                            account: accounts[0], // Ensure account is properly set
                            chain,
 ...
 */
function initializeLiFiForQuotes() {
    if (isConfigured) {
        return;
    }
    try {
 ...
 */
function isLiFiConfigured() {
    return isConfigured;
}
/**
 * Reset configuration state (useful for testing)
 */
function resetLiFiConfig() {
    isConfigured = false;
}
/**
...
async function validateWalletProvider() {
    try {
        ...
        if (provider.selectedAddress) {
            console.log('[LiFi Config] Wallet connected with address:', provider.selectedAddress);
        }
        else {
 ...
 */
async function checkExecutionProviders() {
    try {
        // Import the config to check available providers
        const { config } = await Promise.resolve().then(() => __importStar(require('@lifi/sdk')));
