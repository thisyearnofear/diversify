...
exports.SwapErrorHandler = void 0;
...
class SwapErrorHandler {
    ...
    static isTestnetError(error) {
        const errorMsg = error instanceof Error ? error.message : String(error);
        return (errorMsg.toLowerCase().includes('sepolia') ||
            errorMsg.includes('11142220') ||
            errorMsg.includes('testnet'));
    }
    static shouldSimulateSwap(error, chainId) {
        return chain_detection_service_1.ChainDetectionService.isTestnet(chainId) && this.isTestnetError(error);
    }
}
exports.SwapErrorHandler = SwapErrorHandler;
