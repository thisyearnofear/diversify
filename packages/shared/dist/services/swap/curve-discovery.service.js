...
exports.CurveDiscoveryService = void 0;
...
class CurveDiscoveryService {
    /**
     * Discover all Curve contracts on Arc Testnet
     */
    static async discoverContracts(provider) {
     ...
     */
    static async findSpecificUsdcEurcPool(provider, usdcAddress, eurcAddress) {
        ...
        try {
            ...
            for (const pool of contracts.pools) {
                const hasUsdc = pool.coins.some(coin => coin.toLowerCase() === usdcAddress.toLowerCase());
                const hasEurc = pool.coins.some(coin => coin.toLowerCase() === eurcAddress.toLowerCase());
                if (hasUsdc && hasEurc) {
    ...
    static async findAddressProvider(provider) {
        for (const address of CURVE_DISCOVERY_CONFIG.POTENTIAL_ADDRESS_PROVIDERS) {
            try {
                const contract = new ethers_1.ethers.Contract(address, DISCOVERY_ABIS.ADDRESS_PROVIDER, provider);
    ...
    static async discoverFromAddressProvider(provider, addressProvider, result) {
        const apContract = new ethers_1.ethers.Contract(addressProvider, DISCOVERY_ABIS.ADDRESS_PROVIDER, provider);
        try {
            // Get main registry
            const mainRegistry = await apContract.get_address(CURVE_DISCOVERY_CONFIG.REGISTRY_IDS.MAIN_REGISTRY);
            if (mainRegistry && mainRegistry !== ethers_1.ethers.constants.AddressZero) {
            ...
            if (metaRegistry && metaRegistry !== ethers_1.ethers.constants.AddressZero) {
            ...
            if (exchanges && exchanges !== ethers_1.ethers.constants.AddressZero) {
    ...
    static async discoverPoolsFromRegistry(provider, registry, result) {
        try {
            const registryContract = new ethers_1.ethers.Contract(registry, DISCOVERY_ABIS.REGISTRY, provider);
            ...
            for (let i = 0; i < maxPools; i++) {
                try {
                    ...
                    const validCoins = coins.filter((coin) => coin !== ethers_1.ethers.constants.AddressZero);
    ...
    static findUsdcEurcPoolFromList(pools) {
...
exports.CurveDiscoveryService = CurveDiscoveryService;
//# sourceMappingURL=curve-discovery.service.js.map