...
exports.SynthDataService = void 0;
...
class SynthDataService {
     ...
     */
    static async getPredictions(asset) {
        try {
            const result = await unified_cache_service_1.unifiedCache.getOrFetch(`synth:predictions:${asset}`, async () => {
                const data = await this.makeApiRequest(`${BASE_URL}/insights/prediction-percentiles`, { asset });
                if (!data)
     ...
     */
    static async getVolatility(asset) {
        try {
            const result = await unified_cache_service_1.unifiedCache.getOrFetch(`synth:volatility:${asset}`, async () => {
                const data = await this.makeApiRequest(`${BASE_URL}/insights/volatility`, { asset });
                if (!data)
                    throw new Error('No data from API');
                const mappedData = {
     ...
     */
    static async makeApiRequest(url, params) {
        const headers = {};
        if (API_KEY) {
     ...
     */
    static getFallbackForecast(asset) {
     ...
     */
    static getFallbackVolatility(asset) {
        const fallback = FALLBACK_DATA[asset];
        if (fallback) {
     ...
     */
    static mapStockToSynthAsset(stock) {
        const mapping = {
...
exports.SynthDataService = SynthDataService;
//# sourceMappingURL=synth-data-service.js.map