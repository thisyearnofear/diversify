...
exports.GeminiService = exports.TokenPriceService = exports.ExchangeRateService = exports.WorldBankService = void 0;
...
exports.WorldBankService = {
     ...
     */
    getInflationData: async (year) => {
        const cacheKey = `worldbank-inflation-${year || 'latest'}`;
        // Simple caching without circuit breaker for now
        return unified_cache_service_1.unifiedCache.getOrFetch(cacheKey, () => exports.WorldBankService.fetchInflationData(year), 'moderate');
    },
    /**
     * Internal method to fetch data from World Bank API
     */
    fetchInflationData: async (year) => {
        // Build URL
...
exports.ExchangeRateService = {
    ...
    getExchangeRate: async (fromCurrency, toCurrency) => {
        ...
        return unified_cache_service_1.unifiedCache.getOrFetch(cacheKey, () => exports.ExchangeRateService.fetchExchangeRate(fromCurrency, toCurrency), 'volatile');
    },
    /**
    ...
    fetchExchangeRate: async (fromCurrency, toCurrency) => {
        ...
        try {
            const frankfurterData = await exports.ExchangeRateService.fetchFromFrankfurter(fromCurrency, toCurrency);
            if (frankfurterData) {
     ...
     */
    fetchFromFrankfurter: async (fromCurrency, toCurrency) => {
     ...
     */
    getHistoricalRates: async (fromCurrency, toCurrency, outputSize = 'compact') => {
...
exports.TokenPriceService = {
        ...
        const result = await unified_cache_service_1.unifiedCache.getOrFetch(cacheKey, async () => {
            ...
            if (params.address) {
                ...
                if (typeof defiLlamaPrice === 'number') {
                    return { data: defiLlamaPrice, source: 'defillama' };
                }
            }
            ...
            if (params.symbol) {
                ...
                if (cgId) {
                    ...
                    if (typeof coingeckoPrice === 'number') {
                        return { data: coingeckoPrice, source: 'coingecko' };
                    }
                }
            }
            ...
            if (params.symbol) {
                ...
                if (typeof paprikaPrice === 'number') {
                    return { data: paprikaPrice, source: 'coinpaprika' };
                }
            }
    ...
    async getExpectedAmountOut(params) {
        ...
        try {
            ...
            const fromRate = typeof fromUsd === 'number' ? fromUsd : (config_1.EXCHANGE_RATES[fromToken] ?? 1);
            const toRate = typeof toUsd === 'number' ? toUsd : (config_1.EXCHANGE_RATES[toToken] ?? 1);
     ...
     */
    async fetchDefiLlamaPrice(chainId, address) {
        try {
     ...
     */
    async fetchCoingeckoPrice(id, vsCurrency = 'usd') {
        try {
...
function getFallbackRegionalInflationData(regionCode) {
    const regionCountries = REGION_COUNTRY_MAPPING[regionCode] || [];
    const countries = FALLBACK_INFLATION_DATA.countries.filter(country => regionCountries.includes(country.countryCode));
    return {
...
exports.GeminiService = {
     ...
     */
    analyzeWealthProtection: async (inflationData, userBalance, currentHoldings, config) => {
        try {
