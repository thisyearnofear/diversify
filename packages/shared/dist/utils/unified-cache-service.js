...
exports.unifiedCache = exports.UnifiedCacheService = void 0;
class UnifiedCacheService {
     ...
     */
    async getOrFetch(key, fetchFn, category = 'moderate', forceRefresh = false) {
     ...
     */
    async refresh(key, fetchFn, category = 'moderate') {
        const result = await fetchFn();
        const ttl = this.TTL_CONFIG[category];
        this.set(key, result.data, ttl, result.source, result.etag);
        return result.data;
    ...
    async warmCache(warmupFunctions) {
        ...
        await Promise.all(warmupFunctions.map(async (warmupFn, index) => {
            try {
                await warmupFn();
...
exports.unifiedCache = new UnifiedCacheService();
//# sourceMappingURL=unified-cache-service.js.map