...
exports.macroService = exports.MacroEconomicService = void 0;
...
class MacroEconomicService {
    circuitBreaker = circuit_breaker_service_1.circuitBreakerManager.getCircuit("macro-service", {
        failureThreshold: 3,
        timeout: 30000,
        successThreshold: 2,
    });
     ...
     */
    async getMacroData(countries) {
        const cacheKey = `macro-data-${countries ? countries.sort().join("-") : "all"}`;
        return this.circuitBreaker.callWithFallback(async () => {
            return unified_cache_service_1.unifiedCache.getOrFetch(cacheKey, () => this.fetchFromProxy(countries), "stable");
        }, () => this.getFallbackData(countries));
    }
    /**
     * Helper to get macro data for a specific currency (e.g., 'KES' -> Kenya data)
     */
    async getMacroDataForCurrency(currency) {
        const countryCode = inflation_1.CURRENCY_TO_COUNTRY[currency];
        if (!countryCode)
            return null;
     ...
     */
    calculateStabilityScore(indicator) {
        ...
        const governanceScore = governanceIndicators.length > 0
...
exports.MacroEconomicService = MacroEconomicService;
exports.macroService = new MacroEconomicService();
//# sourceMappingURL=macro-economic-service.js.map