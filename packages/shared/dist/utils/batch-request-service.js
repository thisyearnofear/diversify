...
exports.batchService = exports.BatchRequestService = void 0;
class BatchRequestService {
    pendingBatches = new Map();
    batchTimers = new Map();
    DEFAULT_BATCH_SIZE = 10;
    DEFAULT_MAX_DELAY = 50; // milliseconds
    /**
     * Batch a request for execution
     */
    async batchRequest(endpoint, params, config = {}) {
    ...
    async executeBatch(endpoint, batchKey) {
        ...
        try {
            ...
            // Execute each group
            const results = await Promise.all(groupedRequests.map(group => this.executeGroup(endpoint, group)));
     ...
     */
    async executeGroup(endpoint, requests) {
        ...
        try {
            ...
            return Promise.all(requests.map(request => this.makeSingleRequest(endpoint, request.params)));
        }
    }
    /**
     * Make a single API request
     */
    async makeSingleRequest(endpoint, params) {
     ...
     */
    async makeBatchRequest(endpoint, paramSets) {
     ...
     */
    areRequestsSimilar(params1, params2) {
        ...
        if (commonKeys.length !== keys1.length)
            return false;
     ...
     */
    generateBatchKey(endpoint, params) {
     ...
     */
    scheduleBatchExecution(endpoint, batchKey, delay) {
        const timerKey = `${endpoint}-${batchKey}`;
...
exports.batchService = new BatchRequestService();
