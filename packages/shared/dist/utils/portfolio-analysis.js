...
exports.PortfolioAnalysisUtils = void 0;
exports.createEmptyAnalysis = createEmptyAnalysis;
exports.getTokenInflationRate = getTokenInflationRate;
exports.calculateWeightedInflationRisk = calculateWeightedInflationRisk;
exports.calculateDiversificationScore = calculateDiversificationScore;
exports.getConcentrationRisk = getConcentrationRisk;
exports.calculateProjections = calculateProjections;
exports.generateRebalancingOpportunities = generateRebalancingOpportunities;
exports.generateTargetAllocations = generateTargetAllocations;
exports.analyzePortfolio = analyzePortfolio;
exports.detectGuidedTour = detectGuidedTour;
const token_scoring_1 = require("./token-scoring");
const macro_economic_service_1 = require("./macro-economic-service");
 ...
 */
function getTokenInflationRate(symbol, inflationData) {
    const region = (0, index_1.getTokenRegion)(symbol);
 ...
 */
function calculateWeightedInflationRisk(tokens) {
    if (tokens.length === 0 ||
 ...
 */
function calculateDiversificationScore(tokens, regionalExposure, macroData) {
    if (tokens.length === 0)
        return { score: 0, rating: "Very Poor" };
    const totalValue = tokens.reduce((sum, t) => sum + t.value, 0);
    if (totalValue === 0)
        return { score: 0, rating: "Very Poor" };
    ...
    if (macroData && Object.keys(macroData).length > 0) {
        ...
        regionalExposure.forEach((exposure) => {
            ...
            const regionScores = proxies
                ...
                .filter(Boolean)
                .map((d) => macro_economic_service_1.macroService.calculateStabilityScore(d));
            if (regionScores.length > 0) {
                const avgRegionScore = regionScores.reduce((a, b) => a + b, 0) / regionScores.length;
    ...
    const score = Math.min(100, Math.round(weightedConcentration + tokenBonus + regionBonus + rwaBonus + macroBonus));
 ...
 */
function getConcentrationRisk(regionalExposure) {
    if (regionalExposure.length === 0)
        return "LOW";
    const maxExposure = Math.max(...regionalExposure.map((r) => r.percentage));
    if (maxExposure > CONCENTRATION_THRESHOLDS.HIGH)
        return "HIGH";
    if (maxExposure > CONCENTRATION_THRESHOLDS.MEDIUM)
        return "MEDIUM";
    return "LOW";
}
/**
 ...
 */
function calculateProjections(currentWeightedInflation, optimizedWeightedInflation, totalValue, timeHorizonYears = 3) {
 ...
 */
function generateRebalancingOpportunities(tokens, inflationData, goal = "exploring") {
    const opportunities = [];
    const totalValue = tokens.reduce((sum, t) => sum + t.value, 0);
    if (totalValue === 0)
    ...
    if (goal === "geographic_diversification") {
        ...
        targetTokens = tokens.filter((t) => t.inflationRate <= 6 && // Accept slightly higher inflation for diversification
            (t.region !== "Global" || t.inflationRate <= 2));
    ...
    for (const highToken of highInflationTokens) {
        for (const targetToken of targetTokens) {
            // Skip same token
            if (highToken.symbol === targetToken.symbol)
                continue;
            ...
            const isGoalAligned = (goal === "geographic_diversification" &&
            ...
            if ((inflationDelta > 5 && suggestedAmount > 100) ||
                (isGoalAligned && inflationDelta > 2)) {
                priority = "HIGH";
            }
            else if ((inflationDelta > 3 && suggestedAmount > 50) ||
                isGoalAligned) {
                priority = "MEDIUM";
            }
            opportunities.push({
 ...
 */
function generateTargetAllocations(goal, totalValue, inflationData) {
 ...
 */
function getBestTokenForRegion(region, inflationData, goal = "exploring", riskTolerance = "balanced") {
    ...
    const typedGoal = goal || "exploring";
    const typedRisk = riskTolerance || "balanced";
    const recommendation = (0, token_scoring_1.getBestTokenForRegionDynamic)(region, token_scoring_1.DEFAULT_MARKET_CONTEXT, {
        goal: typedGoal,
 ...
 */
function getAllocationReason(region, goal, inflationData) {
 ...
 */
function analyzePortfolio(portfolio, inflationData, currentGoal = "exploring", macroData) {
    ...
    portfolio.chains.forEach((chain) => {
        chain.balances.forEach((balance) => {
            ...
            const yieldRate = (0, token_scoring_1.getTokenApy)(symbol);
            tokens.push({
                symbol,
                value,
                percentage,
    ...
    const { score: diversificationScore, rating: diversificationRating } = calculateDiversificationScore(tokens, regionalExposure, macroData);
    ...
    const avgYieldRate = portfolio.totalValue > 0
    ...
    const projections = calculateProjections(weightedInflationRisk, optimizedInflationRisk, portfolio.totalValue);
    ...
    if (diversificationScore < 60)
        diversificationTips.push("Aim to have at least 3 different regions in your portfolio.");
    if (missingRegions.length > 0)
        diversificationTips.push(`Consider adding exposure to ${missingRegions
            .slice(0, 2)
            .join(", ")} to improve diversification.`);
    if (concentrationRisk === "HIGH")
        diversificationTips.push("High concentration detected. Consider rebalancing to other regions.");
    if (!hasRWA)
        diversificationTips.push("Add PAXG on Arbitrum as a hedge against currency debasement.");
    ...
    if (macroData && Object.keys(macroData).length > 0) {
        ...
        if (lowStabilityRegions.length > 0) {
            diversificationTips.push(`Macro Risk: ${lowStabilityRegions.join(", ")} exposure carries higher risk due to governance or growth factors.`);
        }
    }
    return {
 ...
 */
function detectGuidedTour(analysis, userGoal, visitedTabs) {
 ...
 * Main Export Object
 */
exports.PortfolioAnalysisUtils = {
    analyzePortfolio,
...
//# sourceMappingURL=portfolio-analysis.js.map