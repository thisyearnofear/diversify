...
exports.marketPulseService = exports.MarketPulseService = void 0;
const synth_data_service_1 = require("../services/synth-data-service");
const macro_economic_service_1 = require("./macro-economic-service");
const intelligence_service_1 = require("../services/ai/intelligence.service");
const macroService = new macro_economic_service_1.MacroEconomicService();
const FICTIONAL_TO_REAL = {
    STARK: {
        proxy: "BTC",
...
class MarketPulseService {
    ...
    static async getMarketPulse() {
        if (this.cache && Date.now() - this.cache.timestamp < this.CACHE_TTL) {
            return this.cache.data;
        }
        // Fetch data with error handling
        const [synthBTC, synthVol, macroData] = await Promise.allSettled([
            synth_data_service_1.SynthDataService.getPredictions("BTC"),
            synth_data_service_1.SynthDataService.getVolatility("BTC"),
     ...
     */
    static generateFallbackSentiment() {
        // Random sentiment between 40 and 60 (neutral range)
        return 40 + Math.random() * 20;
    }
    static calculateWarRisk(data) {
    ...
    static calculateAIMomentum(btc, vol) {
        if (!btc || !vol)
            return 50;
    ...
    static calculateDefenseSpending(warRisk) {
        return 20 + (warRisk * 0.6);
    }
    static generateTriggers(pulse) {
        const triggers = [];
        if (pulse.warRisk > 50) {
            triggers.push({
                stock: "STARK",
    ...
    static async generateIntelligenceItems() {
        ...
        const { inflationService } = await Promise.resolve().then(() => __importStar(require('./improved-data-services')));
        ...
        const guardianInsights = await intelligence_service_1.IntelligenceService.generateGuardianInsights(pulse, inflationResult.data, macroResult.data);
        items.push(...guardianInsights);
        for (const trigger of triggers) {
            const intensity = Math.round(trigger.strength * 100);
            items.push({
                id: `pulse-${trigger.stock}-${new Date().toISOString().split('T')[0]}`, // Daily unique ID
                type: "impact",
        ...
        if (pulse.warRisk > 60) {
            items.push({
                id: `alert-war-${new Date().toISOString().split('T')[0]}`,
                type: "alert",
        ...
        if (pulse.btcChange24h > 3 || pulse.btcChange24h < -3) {
            items.push({
                id: `alert-btc-${new Date().toISOString().split('T')[0]}`,
                type: "news",
        ...
        // Final deduplication pass using the same logic as Guardian insights
        return intelligence_service_1.IntelligenceService.deduplicateInsights(sorted);
    }
}
exports.MarketPulseService = MarketPulseService;
exports.marketPulseService = MarketPulseService;
//# sourceMappingURL=market-pulse-service.js.map