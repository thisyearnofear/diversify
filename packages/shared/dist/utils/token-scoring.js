...
exports.TokenScoringUtils = exports.TOKEN_PERFORMANCE = exports.DEFAULT_MARKET_CONTEXT = void 0;
exports.calculateRealYield = calculateRealYield;
exports.scoreTokens = scoreTokens;
exports.getTokenApy = getTokenApy;
exports.calculateOpportunityCost = calculateOpportunityCost;
exports.getBestTokenForRegionDynamic = getBestTokenForRegionDynamic;
// ============================================================================
// DEFAULT MARKET CONTEXT (Fallback when real-time data unavailable)
// ============================================================================
exports.DEFAULT_MARKET_CONTEXT = {
    treasuryYield: 4.5,
    inflation: 3.2,
    goldPrice: 2650,
    goldYtdChange: 15,
 ...
 */
function calculateRealYield(treasuryYield, inflation) {
 ...
 */
function calculateYieldScore(tokenApy, maxApy) {
    if (maxApy === 0)
        return 0;
 ...
 */
function calculateInflationHedgeScore(token, marketContext, userContext) {
    let score = 0;
 ...
 */
function calculateRealYieldScore(token, marketContext) {
    const realYield = calculateRealYield(marketContext.treasuryYield, marketContext.inflation);
    if (token.symbol === 'PAXG' && token.ytdReturn === 0) {
        // Non-yielding asset
        if (realYield > 2)
            return -30; // Strong penalty when Treasuries pay 2%+
        if (realYield > 1)
            return -15; // Moderate penalty
        if (realYield < 0)
            return 15; // Bonus when real yields negative
    }
    if (['USDY', 'SYRUPUSDC'].includes(token.symbol)) {
        // Yield-bearing assets
        if (realYield > 2)
            return 20; // Bonus when real yields strong
        if (realYield > 0)
            return 10; // Small bonus
    }
    return 0;
}
/**
 ...
 */
function calculatePerformanceScore(token) {
    return token.ytdReturn * 2; // 1% return = 2 points
}
/**
 * Score based on risk-adjusted returns (Sharpe ratio)
 */
function calculateRiskAdjustedScore(token, userContext) {
 ...
 */
function scoreTokens(symbols, marketContext = exports.DEFAULT_MARKET_CONTEXT, userContext, inflationData) {
    const scores = [];
    // Find max APY for normalization
    const maxApy = Math.max(...symbols.map(s => getTokenApy(s)));
    for (const symbol of symbols) {
        const token = exports.TOKEN_PERFORMANCE[symbol];
        if (!token)
            continue;
        const breakdown = {
 ...
 */
function generateReasoning(symbol, breakdown, marketContext, userContext) {
    const reasoning = [];
    const realYield = calculateRealYield(marketContext.treasuryYield, marketContext.inflation);
    if (symbol === 'PAXG') {
        if (marketContext.inflation > 4) {
    ...
    if (breakdown.riskAdjustedScore > 50) {
        reasoning.push(`Excellent risk-adjusted returns (Sharpe: ${exports.TOKEN_PERFORMANCE[symbol]?.sharpeRatio.toFixed(2)})`);
    }
    return reasoning;
}
// ============================================================================
 ...
 */
function calculateOpportunityCost(tokenSymbol, alternatives, investmentAmount = 10000, marketContext = exports.DEFAULT_MARKET_CONTEXT) {
 ...
 */
function getBestTokenForRegionDynamic(region, marketContext, userContext, inflationData) {
...
// EXPORT
// ============================================================================
exports.TokenScoringUtils = {
    calculateRealYield,
    scoreTokens,
    calculateOpportunityCost,
    getBestTokenForRegionDynamic,
    DEFAULT_MARKET_CONTEXT: exports.DEFAULT_MARKET_CONTEXT,
};
exports.default = exports.TokenScoringUtils;
//# sourceMappingURL=token-scoring.js.map