...
import {
    ...
    isTradeableCompany,
    getTradingTokenAddress,
    getFictionalCompany,
} from '../../../config/emerging-markets';
import { NETWORKS, TOKEN_METADATA } from '../../../config';
...
export class EmergingMarketsStrategy extends BaseSwapStrategy {
    private readonly AMM_ADDRESS = EMERGING_MARKETS_CONFIG.ammAddress;
    private readonly WETH_ADDRESS = EMERGING_MARKETS_CONFIG.wethAddress;
    ...
    supports(params: SwapParams): boolean {
        ...
        if (params.fromChainId !== EMERGING_MARKETS_CONFIG.chainId) {
            return false;
        }
        ...
        const isFromCompany = isTradeableCompany(params.fromToken);
        const isToCompany = isTradeableCompany(params.toToken);
    ...
    async validate(params: SwapParams): Promise<boolean> {
        ...
        const tokenAddress = getTradingTokenAddress(companySymbol);

        if (!tokenAddress) {
            throw new Error(
                `Company ${companySymbol} is not available on ${ChainDetectionService.getNetworkName(params.fromChainId)}`
            );
        }
        ...
        try {
            ...
            const amm = new ethers.Contract(this.AMM_ADDRESS, AMM_ABI, provider);
    ...
    async getEstimate(params: SwapParams): Promise<SwapEstimate> {
        ...
        const company = getFictionalCompany(companySymbol);
        ...
        const gasCostEstimate = ethers.BigNumber.from('350000');
    ...
    async execute(params: SwapParams, callbacks?: SwapCallbacks): Promise<SwapResult> {
        ...
        try {
            ...
            if (isFromCELO) {
                ...
                const tokenContract = new ethers.Contract(company.tokenAddress, ERC20_ABI, signer);
